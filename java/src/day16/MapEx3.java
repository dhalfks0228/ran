...
public class MapEx3 {
	...
	public static void main(String[] args) {
		...
		// 메뉴를 입력 : Scanner를 이용하여 입력받은 메뉴를 저장
		menu = scan.nextInt();
		
		// 입력받은 메뉴에 따라 기능을 처리 : switch문
		// case 1~5, default : 잘못된 기능.
		switch (menu) {
		case 1:		
			/* 1번 기능 : 학생 과목과 성적을 입력받고, 입력받은 정보를 map에 추가
			 1. 과목과 성적을 입력받고, 
			 - 과목과 성적을 저장하기 위한 변수
			 - 과목과 성적을 Scanner로 입력
			 2. map에 추가.
		...
		case 2:
			 ...
			 * - Iterator를 이용하여 출력
			 * - Map에 key(subject)값을 set으로 만든 후 만들어진 set의 Iterator를 가져옴.*/
			print(map);
			break;
		case 3:
			/* 3번 기능 : 과목을 입력받고, 입력받은 과목의 성적을 출력
			 * - 과목 입력
			 * - 입력한 과목의 성적 출력
			 * 단, 해당 과목이 없으면 없다고 출력
			 * - map에서 과목에 대한 성적을 조회*/
	...
	public static void insert(Scanner scan, HashMap<String, Integer> map) {
		System.out.print("과목과 성적을 입력(ex: 영어 100) : ");
	...
	public static void print(Scanner scan, HashMap<String, Integer> map ) {
		...
		Integer tmp = map.get(subject);
			if(tmp == null) {
				...
				System.out.println(subject + " : " + tmp); // 그 과목 하나만 출력.
			}
	}
	public static void modify(Scanner scan, HashMap<String, Integer>map) {
		...
		if(tmp2 == null) {
			...
			map.put(subject, score);
			System.out.println(subject + "성적이 " + score + "으로 수정되었습니다.");
		}
	}
}
